...
##【 IMPULSE 】
  ...
  # /* 初回起動時、管理スコアをリセットしコンフィグを読み込む */
    execute unless entity @a run function cronica_dev:system/common/scoreboard/config/remove
    function cronica_dev:system/common/scoreboard/config/setup

  # 実績リセット
  # /* リロード時、エラー回避のため取得済みの実績をすべて解除 */
    execute as @a run function cronica_dev:player/status/detection/advancements/revoke
