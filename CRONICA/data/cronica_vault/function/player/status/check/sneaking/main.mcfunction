...
# 使用終了検知
execute if score @s[scores={SCORE.cronica.STATUS.Sneaking=1..}] SCORE.cronica.STATUS.Sneaked matches 0 run function cronica:player/status/check/sneaking/finish
scoreboard players set @s SCORE.cronica.STATUS.Sneaked 0

#【 カテゴリ別処理 】
# 各種処理へ分岐する
# スニークでの処理の場合右手に持っているものがスニークの要求がないものの可能性もあるため、両方を確認する
execute as @s[tag=TAG.cronica.STATUS.Sneaking] run function cronica:player/status/check/sneaking/category/data_get_main
execute as @s[tag=TAG.cronica.STATUS.Sneaking] run function cronica:player/status/check/sneaking/category/data_get_off
# =================================================================================================
