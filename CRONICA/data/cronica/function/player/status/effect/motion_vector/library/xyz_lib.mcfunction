...
##【 IMPULSE 】
  ...
  ## 指定の力で視点方向に対象を飛ばす
    ...
    # ベクトルごとの強さ計算
      ...
      $execute store result storage cronica:temp MotionVector.Macro.$(Vector)1 int 0.0001 run scoreboard players operation #VectorMaster_$(Vector) SCORE.cronica.STATUS.MotionVector = #VectorMaster_$(Vector) SCORE.cronica.STATUS.MotionVector
