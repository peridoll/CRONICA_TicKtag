...
##【 REPEAT 】
  ...
  ## 特定関数呼び出し
    ...
    # 中断処理
      execute as @a[scores = {SCORE.cronica.TIMER.flying_feather.ActivateTime = 0..19 }] at @s unless block ~ ~2.1 ~ #core:no_collision run \
        function cronica:player/character/ikaros/select/flying_feather/library/stop_ceiling
      execute as @a[scores = {SCORE.cronica.TIMER.flying_feather.ActivateTime = 0.. }, tag = TAG.cronica.STUNNING] run \
