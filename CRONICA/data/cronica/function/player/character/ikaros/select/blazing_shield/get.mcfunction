...
##【 IMPULSE 】
  ...
  ## アイテムモデル変更
    ...
    # 基本形態
      execute if score @s SCORE.cronica.MODE.blazing_shield matches 0 run function cronica:player/status/common_lib/item_modify {ItemID: "blazing_shield", Model: "common/default"}

    # 使用済み
      execute if score @s SCORE.cronica.MODE.blazing_shield matches -1 run function cronica:player/status/common_lib/item_modify {ItemID: "blazing_shield", Model: "common/used"}
