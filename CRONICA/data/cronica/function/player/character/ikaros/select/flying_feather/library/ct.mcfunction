...
##【継続処理】
  ...
  # 初回スタック進行
    execute \
        as @a[scores = {SCORE.cronica.TIMER.flying_feather.CoolTime = 0..}] \
          if score @s SCORE.cronica.TIMER.flying_feather.CoolTime.Stack matches 0 \
      run \
        function cronica:player/status/common_lib/ct/progress {MasterID: "ikaros", MasterType: "select", ItemID: "flying_feather"}
  ...
  # スタック進行
    execute \
        as @a[scores = {SCORE.cronica.TIMER.flying_feather.CoolTime = 0..}] \
          if score @s SCORE.cronica.TIMER.flying_feather.CoolTime.Stack >= @s SCORE.cronica.TIMER.flying_feather.CoolTime.Stack.Goal \
          at @s \
      run \
        function cronica:player/status/common_lib/ct/progress {MasterID: "ikaros", MasterType: "select", ItemID: "flying_feather"}
  ...
  # チャージ完了
    execute \
        as @a[scores = {SCORE.cronica.TIMER.flying_feather.CoolTime = 0..}] \
          if score @s SCORE.cronica.TIMER.flying_feather.CoolTime >= @s SCORE.cronica.TIMER.flying_feather.CoolTime.Goal \
          at @s \
      run \
