...
# チャージ完了
execute as @a[scores={SCORE.cronica.TIMER.flying_feather.CoolTime=0..}] if score @s SCORE.cronica.TIMER.flying_feather.CoolTime >= @s SCORE.cronica.TIMER.flying_feather.CoolTime.Goal at @s run function cronica:player/status/common_lib/ct/finish {MasterID: "ikaros", MasterType: "select", ItemID: "flying_feather"}

# ループ処理
execute if entity @a[scores={SCORE.cronica.TIMER.flying_feather.CoolTime=0..}] run schedule function cronica:player/character/ikaros/select/flying_feather/library/ct 1t
# =================================================================================================
