...
##【 IMPULSE 】
  ...
  ## アイテムモデル変更
    ...
    # 基本形態
      execute if score @s SCORE.cronica.MODE.spinning_slash matches 0 run function cronica:player/status/common_lib/item_modify {ItemID: "spinning_slash", Model: "common/default"}

    # 使用済み
      execute if score @s SCORE.cronica.MODE.spinning_slash matches -1 run function cronica:player/status/common_lib/item_modify {ItemID: "spinning_slash", Model: "common/used"}

    # 使用不可
      execute if score @s SCORE.cronica.MODE.spinning_slash matches -2 run function cronica:player/status/common_lib/item_modify {ItemID: "spinning_slash", Model: "common/ban"}
