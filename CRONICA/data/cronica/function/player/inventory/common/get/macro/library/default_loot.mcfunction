  ...
  ## データ管理
    ...
    # スコア管理
      ...
      # 基本スタック数設定
        ...
        $scronicaboard objectives add SCORE.cronica.INVENTORY.$(ItemID).StackCount dummy
        $scronicaboard players set @s SCORE.cronica.INVENTORY.$(ItemID).StackCount 1

      # コンフィグ読み込み
        # 参照コンフィグから基本スタック数を取得し、スタック数スコアに設定する
        # "アイテムID".CustomData.DefaultStack
        $scronicaboard objectives add SCORE.cronica.INVENTORY.$(ItemID).DefaultStack dummy
        $execute store result scronica @s SCORE.cronica.INVENTORY.$(ItemID).DefaultStack run \
        ...
        $execute if score @s SCORE.cronica.INVENTORY.$(ItemID).DefaultStack matches 1.. run \
          scronicaboard players operation @s SCORE.cronica.INVENTORY.$(ItemID).StackCount = @s SCORE.cronica.INVENTORY.$(ItemID).DefaultStack
        $scronicaboard objectives remove SCORE.cronica.INVENTORY.$(ItemID).DefaultStack
  ...
  ## アイテム入手
    ...
    # スロット登録済みアイテム入手 & 単一アイテム入手
      execute if entity @s[ tag =! TAG.cronica.INVENTORY.ResetSavingSlot ] run \
        function cronica:player/inventory/common/get/macro/library/loading_slot with storage cronica:temp CharacterGet
    ...
    # アイテム情報記入
      function cronica:player/inventory/common/get/macro/library/refined_lore with storage cronica:temp CharacterGet
