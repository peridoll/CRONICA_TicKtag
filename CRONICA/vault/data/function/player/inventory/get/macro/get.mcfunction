  ...
  ## アイテム取得
    ...
    # 未所持の場合、アイテムを新たに入手
      $execute if entity @s[tag =! TAG.cronica.INVENTORY.$(ItemID).Has] run \
        function cronica:player/inventory/get/macro/library/default_loot with storage cronica:temp CharacterGet

    # アイテムモデル更新
      function cronica:player/inventory/get/macro/library/default_model with storage cronica:temp CharacterGet
